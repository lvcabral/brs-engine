...
|setCaptionStyle(`style`) |Configures the closed caption style options. |<ul><li>`style` (object[]): The caption styles array with each option as `{id: string, style: string}`, see options in [Roku Documentation](https://developer.roku.com/dev/docs/ifdeviceinfo#getcaptionsoptionoption-as-string-as-string).</li></ul>|
...
|sendKeyDown(`key`, `remote?`, `index?`)|Sends a remote control key down event to the engine|<ul><li>`key` (string): one of valid key codes (see [Roku doc](https://developer.roku.com/dev/docs/onkeyevent)).</li><li>`remote` (number): one of valid remote types (see [/src/api/common.ts](/src/api/common.ts)) default is `ECP`.</li><li>`index` (number): The index of the remote control, default is 0.</li>|
|sendKeyUp(`key`, `remote?`, `index?`)|Sends a remote control key up event to the engine|<ul><li>`key` (string): one of valid key codes (see [Roku doc](https://developer.roku.com/dev/docs/onkeyevent)).</li><li>`remote` (number): one of valid remote types (see [/src/api/common.ts](/src/api/common.ts)) default is `ECP`.</li><li>`index` (number): The index of the remote control, default is 0.</li>|
|sendKeyPress(`key`, `delay?`, `remote?`, `index?`)|Sends a remote control key press event to the engine|<ul><li>`key` (string): one of valid key codes (see [Roku doc](https://developer.roku.com/dev/docs/onkeyevent)).</li><li>`delay` (number): the delay (in milliseconds) between sending Key Up and Key Down (default is 300ms).<li>`remote` (number): one of valid remote types (see [/src/api/common.ts](/src/api/common.ts)) default is `ECP`.</li><li>`index` (number): The index of the remote control, default is 0.</li>|
...
|debug(`command`)|Sends a debug command to the Engine|<ul><li>`command` (string): the Micro Debugger can be enabled sending `break` command, and after that, any valid BrightScript expression or [debug commands](https://developer.roku.com/dev/docs/debugging#brightscript-console-port-8085-commands) can be sent. You can also send `pause` to interrupt the interpreter, for instance, when the app loses focus. The command `cont` restarts the app. You can use this method on the browser console to debug your app.|
