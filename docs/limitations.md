...
* **Roku SceneGraph** SDK components are currently being implemented as an [extension](./extensions.md) published separately as the NPM package [brs-scenegraph](https://www.npmjs.com/package/brs-scenegraph), so far it supports:
  ...
  * **Component Libraries** are supported, both declared in XML (`<ComponentLibrary id="MyLib" uri="..."/>`, where the `id` is the namespace) and created at runtime via `CreateObject("roSGNode", "ComponentLibrary")`. Reference their components as `MyLib:ComponentName`. The `uri` may point to a local volume (`pkg:/`, `tmp:/`, `ext1:/`) or a remote `http(s)://` package (the configured CORS proxy is honored). Current limitations:
