...
* **Node identity, cloning and render-thread-only APIs** differ from a device in three measured cases (probes: `test/simulator/probes/node-container-copy-probe`, `clone-and-setref-probe`, `clone-basetype-probe`):
  * `roUtils.IsSameObject()` compares the engine's node instance, so it returns `true` for two values that refer to the same node. A device mints a fresh BrightScript handle on *every* node retrieval (a `node` field read, `getField`, `findNode`, `getChild`, `getParent`, even a node returned by `roUtils.DeepCopy`), so it answers `false` in almost every real case. **Use `isSameNode()` for node identity** — it matches a device. Copy behavior itself is correct: reading an `assocarray`/`array` field, `roUtils.DeepCopy` and `moveIntoField` all carry the node over unchanged, and `clone` shares node-valued fields; only the identity *report* differs.
  * `clone()` never re-runs a component's `init()`, so the clone's script scope is empty and a `callFunc` on it reads `invalid` for anything the component cached in `m` — this matches a device. Note the device quirk the engine now reproduces: a custom component whose **root** built-in base is `Node` clones to a **bare `Node`** (subtype, declared and `addField` fields, and functions all dropped, leaving only `Node`'s four fields with their current values); rooted at any other built-in — `Group`, `Label`, `ContentNode` — the clone keeps its subtype, fields and functions (a component extending another custom component follows its root).
  * The engine runs `source/` code and the SceneGraph tree on **one thread**, so it has no main-vs-render ScriptEngine split. APIs a device restricts to the render thread are therefore accepted from `Main` — notably `setRef()`, `getRef()` and `canGetRef()`, which a device fails (returning `false`, leaving the field unassigned) when called from the main thread. Their render-thread semantics match a device exactly.
...
* The new **`roWebSocket`** component (Roku OS 16.0) performs real WebSocket network I/O on **both** packages, unlike the raw-socket components above:
  * On the Node.js/CLI package, a per-socket helper process (backed by the `ws` package) gives full protocol fidelity, including real Ping/Pong control frames (`SendPing`/`SendPong`/`PingTest`/`SetAutoPingReply`), custom handshake headers, HTTP Basic-Auth credentials (`SetUserAndPassword`), and TLS peer/host verification toggles.
  * On the browser package, the real `WebSocket` connection is opened on the main thread (the interpreter's own thread cannot service its callbacks) and bridged to the worker via shared memory. Browsers give script no way to set handshake headers or credentials and no way to observe or originate Ping/Pong control frames, so on that package: `SetUserAndPassword`, custom `ifHttpAgent` headers, and `EnablePeerVerification`/`EnableHostVerification` are accepted but have no effect; `SendPing`/`SendPong`/`PingTest` are no-ops (the browser still auto-replies to an incoming Ping, invisibly to script, regardless of `SetAutoPingReply`).
  * `GetBuffered()` always returns `0` and `SetFragmentSize()`/`GetMsgSendBufferSize()`/`GetMsgRecvBufferSize()` are stored but not enforced — neither platform's transport models outgoing buffering or manual frame fragmentation.
