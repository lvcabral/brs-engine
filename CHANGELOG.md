...
* Changes on `brs-scenegraph` package:
  ...
  * (rsg) Added the Roku OS 15.3 `captionRenderArea` associative-array field to the `Video` node, so apps that position captions for preview/non-full-screen playback can set and read it back.
