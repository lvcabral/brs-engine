...
describe("RoRegex", () => {
    ...
    describe("isMatch", () => {
        it("matches strings with case sensitive flag", async () => {
            let regx = new RoRegex(new BrsString("hello_[0-9]*_world"));
            let isMatch = regx.getMethod("isMatch");
            expect(isMatch).toBeTruthy();

            let str1 = await isMatch.call(interpreter, new BrsString("hello_123_world"));
            expect(str1.kind).toBe(ValueKind.Boolean);
            expect(str1).toBe(BrsBoolean.True);

            let str2 = await isMatch.call(interpreter, new BrsString("HeLlO_123_WoRlD"));
            expect(str2.kind).toBe(ValueKind.Boolean);
            expect(str2).toBe(BrsBoolean.False);
        });

        it("matches strings with case insensitive flag", async () => {
            ...
            let str2 = await isMatch.call(interpreter, new BrsString("HeLlO_XXX_WoRlD"));
            expect(str2.kind).toBe(ValueKind.Boolean);
            expect(str2).toBe(BrsBoolean.False);
        });

        it("matches a string in ISO8601 format", async () => {
            ...
            let str1 = await isMatch.call(interpreter, new BrsString("P3Y6M4DT12H30M5S"));
            expect(str1.kind).toBe(ValueKind.Boolean);
            expect(str1).toBe(BrsBoolean.True);

            let str2 = await isMatch.call(interpreter, new BrsString("P4Y5M2DT10H15M5.6S"));
            expect(str2.kind).toBe(ValueKind.Boolean);
    ...
    describe("match", () => {
        it("doesn't match string", async () => {
            let rgx = new RoRegex(new BrsString("(a|(z))(bc)"));
            let match = rgx.getMethod("match");
            expect(match).toBeTruthy();

            let result = await match.call(interpreter, new BrsString(""));
            let count = await result.getMethod("count").call(interpreter);
            expect(result.kind).toBe(ValueKind.Object);
            expect(result.getComponentName()).toBe("roArray");
            expect(count.value).toBe(0);
        });

        it("matches groups in string", async () => {
            let rgx = new RoRegex(new BrsString("(a|(z))(bc)"));
            let match = rgx.getMethod("match");
            expect(match).toBeTruthy();

            let result = await match.call(interpreter, new BrsString("abcd"));
            let count = await result.getMethod("count").call(interpreter);
            expect(result.kind).toBe(ValueKind.Object);
            expect(result.getComponentName()).toBe("roArray");
        ...
        it("matches string in hh:mm:ss.ms date format", async () => {
            let regx = new RoRegex(
            ...
            let result = await match.call(interpreter, new BrsString("10:35:15.123"));
            let count = await result.getMethod("count").call(interpreter);
            expect(result.kind).toBe(ValueKind.Object);
            expect(result.getComponentName()).toBe("roArray");
        ...
        it("matches strings using beginning and end of input", async () => {
            let regx = new RoRegex(
            ...
            let result = await match.call(interpreter, new BrsString("http://regex101.com"));
            let count = await result.getMethod("count").call(interpreter);
            expect(result.kind).toBe(ValueKind.Object);
            expect(result.getComponentName()).toBe("roArray");
            ...
            result = await match.call(interpreter, new BrsString("https://regex101.com"));
            expect(result.kind).toBe(ValueKind.Object);
            expect(result.getComponentName()).toBe("roArray");
            expect(result.get(new Int32(1)).value).toBe("https:"); //Protocol

            result = await match.call(interpreter, new BrsString("https:/regex101.com")); //Missing slash
            count = await result.getMethod("count").call(interpreter);
            expect(result.kind).toBe(ValueKind.Object);
            expect(result.getComponentName()).toBe("roArray");
    ...
    describe("replace", () => {
        it("replaces first matched instance from string", async () => {
            let rgx = new RoRegex(new BrsString("-"));
            let replace = rgx.getMethod("replace");
            expect(replace).toBeTruthy();

            let result = await replace.call(
                interpreter,
        ...
        it("replaces string by using positional replacement patterns", async () => {
    ...
    describe("replaceAll", () => {
        it("replaces all matched instances from string", async () => {
            let rgx = new RoRegex(new BrsString("-"));
            let replaceAll = rgx.getMethod("replaceall");
            expect(replaceAll).toBeTruthy();

            let result = await replaceAll.call(
                interpreter,
                new BrsString("2010-01-01"),
                new BrsString(".")
            );
    ...
    describe("split", () => {
        it("splits in the correct number of items", async () => {
            let rgx = new RoRegex(new BrsString(","));
            let split = rgx.getMethod("split");
            expect(split).toBeTruthy();

            let result = await split.call(interpreter, new BrsString("one, two, three, four"));
            let count = await result.getMethod("count").call(interpreter);
            expect(result.kind).toBe(ValueKind.Object);
            expect(count.value).toBe(4);
        });
    ...
    describe("matchAll", () => {
        it("matches all patterns in the string", async () => {
            let rgx = new RoRegex(new BrsString("\\d+"));
            let matchAll = rgx.getMethod("matchall");
            expect(matchAll).toBeTruthy();

            let result = await matchAll.call(interpreter, new BrsString("123 456 789"));
            let count = await result.getMethod("count").call(interpreter);
