...
describe("Timespan", () => {
    ...
    describe("methods", () => {
        ...
        describe("totalmilliseconds", () => {
            it("returns milliseconds from marked time until now", async () => {
                let advanceTime = 50000;
                let totalmilliseconds = ts.getMethod("totalmilliseconds");

                clock.tick(advanceTime);

                let result = await totalmilliseconds.call(interpreter);
        ...
        describe("totalseconds", () => {
            it("returns seconds from marked time until now", async () => {
                let advanceTime = 50000;
                let totalseconds = ts.getMethod("totalseconds");

                clock.tick(advanceTime);

                let result = await totalseconds.call(interpreter);
        ...
        describe("getsecondstoiso8601date", () => {
            it("returns seconds from now until a valid ISO8601 date", async () => {
                ...
                let result1 = await getsecondstoiso8601date.call(
                    interpreter,
                    new BrsString(dateToParse1)
                );
                let result2 = await getsecondstoiso8601date.call(
                    interpreter,
            ...
            it("accepts other ISO8601 formats and returns correct seconds", async () => {
                let getsecondstoiso8601date = ts.getMethod("getsecondstoiso8601date");

                let dateToParse = "2020-05-25";

                let result = await getsecondstoiso8601date.call(
                    interpreter,
                    new BrsString(dateToParse)
                );
                expect(getsecondstoiso8601date).toBeTruthy();
                expect(result).toEqual(new Int32(43292429));
            });

            it("returns 2077252342 for strings that can't be parsed", async () => {
                let getsecondstoiso8601date = ts.getMethod("getsecondstoiso8601date");
