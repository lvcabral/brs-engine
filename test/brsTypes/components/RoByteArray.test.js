    ...
    describe("ifArray methods", () => {
        ...
        describe("peek", () => {
            it("returns the value at the highest index", async () => {
                ...
                expect(await peek.call(interpreter)).toEqual(new Int32(3));
            });
        ...
        describe("pop", () => {
            it("returns and removes the value at the highest index", async () => {
                ...
                expect(await pop.call(interpreter)).toEqual(new Int32(3));
                expect(arr.elements).toEqual(new Uint8Array([1, 2]));
            });

            it("returns and removes the value of the last element", async () => {
                let arr = new RoByteArray(new Uint8Array([1]));

                let pop = arr.getMethod("pop");
                expect(pop).toBeTruthy();
                expect(await pop.call(interpreter)).toEqual(new Int32(1));
        ...
        describe("push", () => {
            it("appends a value to the end of the array", async () => {
                ...
                expect(await push.call(interpreter, new Int32(3))).toBe(BrsInvalid.Instance);
        ...
        describe("shift", () => {
            it("returns and removes the value at the lowest index", async () => {
                ...
                expect(await shift.call(interpreter)).toEqual(new Int32(1));
            ...
            it("returns and removes the value of the last element", async () => {
                ...
                let shift = arr.getMethod("shift");
                expect(shift).toBeTruthy();
                expect(await shift.call(interpreter)).toEqual(new Int32(3));
                expect(arr.elements).toEqual(new Uint8Array(0));
            });
        ...
        describe("unshift", () => {
            it("inserts a value at the beginning of the array", async () => {
                ...
                expect(await unshift.call(interpreter, new Int32(1))).toBe(BrsInvalid.Instance);
                expect(arr.elements).toEqual(new Uint8Array([1, 2, 3]));
            });
        });
        ...
        describe("ifByteArray methods", () => {
            ...
            describe("toHexString", () => {
                it("converts a byte array to a hex string", async () => {
                    ...
                    let result = await toHexString.call(interpreter);
                    expect(result).toEqual(new BrsString("AA010203FF"));
                });
            });
            describe("getSignedByte", () => {
                it("returns the signed byte at the specified index", async () => {
                    ...
                    expect(await getSignedByte.call(interpreter, new Int32(0))).toEqual(
                        new Int32(0)
                    );
                    expect(await getSignedByte.call(interpreter, new Int32(1))).toEqual(
                        new Int32(127)
                    );
                    expect(await getSignedByte.call(interpreter, new Int32(2))).toEqual(
                        new Int32(-128)
                    );
                    expect(await getSignedByte.call(interpreter, new Int32(3))).toEqual(
                        new Int32(-1)
                    );
                });
            });
            describe("getSignedLong", () => {
                it("returns the signed long at the specified index", async () => {
                    let src = new RoByteArray(
                        new Uint8Array([
                    ...
                    expect(await getSignedLong.call(interpreter, new Int32(0))).toEqual(
                        new Int32(0)
                    );
                    expect(await getSignedLong.call(interpreter, new Int32(1))).toEqual(
                        new Int32(58655615)
                    );
                    expect(await getSignedLong.call(interpreter, new Int32(2))).toEqual(
                        new Int32(-2147483648)
                    ); // 128 shifted left 24 places
                    expect(await getSignedLong.call(interpreter, new Int32(3))).toEqual(
                        new Int32(-1)
                    ); // 255 repeated 4 times is -1 in two's complement
                });
            });
            describe("getCRC32", () => {
                it("returns the CRC32 of the byte array", async () => {
                    let src = new RoByteArray(new Uint8Array([104, 101, 108, 108, 111]));
                    let getCRC32 = src.getMethod("getCRC32");
                    expect(getCRC32).toBeTruthy();

                    let result = await getCRC32.call(interpreter);
                    expect(result).toEqual(new Int32(907060870));
                });
            });
            describe("isLittleEndianCPU", () => {
                it("returns true if the CPU is little-endian", async () => {
                    let src = new RoByteArray();
                    let isLittleEndianCPU = src.getMethod("isLittleEndianCPU");
                    expect(isLittleEndianCPU).toBeTruthy();

                    let expected = os.endianness() === "LE";
                    let result = await isLittleEndianCPU.call(interpreter);
