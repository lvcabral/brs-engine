                ...
                `<Component: roAssociativeArray> =
{
    ...
    describe("get", () => {
        it("returns value given a key it contains", async () => {
            let aa = new RoAssociativeArray([
        ...
        it("returns 'invalid' given a key it doesn't contain", async () => {
            let aa = new RoAssociativeArray([]);
    ...
    describe("set", () => {
        it("sets values with new keys", async () => {
            let aa = new RoAssociativeArray([
                { name: new BrsString("foo"), value: new Int32(-99) },
            ]);
        ...
        it("overwrites values with existing keys", async () => {
            let aa = new RoAssociativeArray([
                { name: new BrsString("foo"), value: new Int32(-99) },
            ]);
    ...
    describe("methods", () => {
        ...
        describe("delete", () => {
            it("deletes a given item in the associative array", async () => {
                ...
                expect(await deleteCall.call(interpreter, new BrsString("foo"))).toBe(
                    BrsBoolean.True
                );
                expect(await deleteCall.call(interpreter, new BrsString("baz"))).toBe(
                    BrsBoolean.False
                );
        ...
        describe("addreplace", () => {
            it("adds new elements to the associative array", async () => {
                ...
                expect(
                    await addreplace.call(interpreter, new BrsString("letter2"), new BrsString("b"))
            ...
            it("replaces the value of known elements in the associative array", async () => {
                ...
                expect(addreplace).toBeTruthy();
                expect(
                    await addreplace.call(interpreter, new BrsString("letter1"), new BrsString("c"))
                ).toBe(BrsInvalid.Instance);
        ...
        describe("count", () => {
            it("returns the number of items in the associative array", async () => {
                ...
                let result = await count.call(interpreter);
        ...
        describe("doesexist", () => {
            it("returns true when an item exists in the array", async () => {
                ...
                let result = await doesexist.call(interpreter, new BrsString("letter2"));
            ...
            it("returns false when an item doesn't exist in the array", async () => {
                ...
                let doesexist = aa.getMethod("doesexist");
                let result = await doesexist.call(interpreter, new BrsString("letter3"));
                expect(result.kind).toBe(ValueKind.Boolean);
                expect(result).toBe(BrsBoolean.False);
            });
            it("returns true even when the value of an item contains invalid in the array", async () => {
                let aa = new RoAssociativeArray([
                    { name: new BrsString("letter1"), value: BrsInvalid.Instance },
                ]);

                let doesexist = aa.getMethod("doesexist");
                let result = await doesexist.call(interpreter, new BrsString("letter1"));
                expect(result.kind).toBe(ValueKind.Boolean);
                expect(result).toBe(BrsBoolean.True);
            });
        });
        ...
        describe("append", () => {
            it("appends a new associative array to an existing one", async () => {
                ...
                let result = await append.call(interpreter, aa2);
        ...
        describe("setModeCaseSensitive", () => {
            it("looks for key using case insensitive by default", async () => {
                ...
                let result = await lookup.call(interpreter, new BrsString("KeY1"));
                expect(result).toBe(v1);

                result = await doesexist.call(interpreter, new BrsString("KeY1"));
                expect(result.kind).toBe(ValueKind.Boolean);
                expect(result).toBe(BrsBoolean.True);
            ...
            it("changes lookups to case sensitive mode", async () => {
                let v1 = new BrsString("value1");
                ...
                let result1 = await lookup.call(interpreter, new BrsString("KeY1"));
                expect(result1).toBe(v1);

                result1 = await doesexist.call(interpreter, new BrsString("key1"));
                expect(result1.kind).toBe(ValueKind.Boolean);
                expect(result1).toBe(BrsBoolean.True);

                // Turn on case sensitive mode
                await setModeCaseSensitive.call(interpreter);

                // Add a new key aftet case mode changed
                await addreplace.call(interpreter, new BrsString("KeY2"), v2);

                let result2 = await doesexist.call(interpreter, new BrsString("key2"));
                expect(result2.kind).toBe(ValueKind.Boolean);
                expect(result2).toBe(BrsBoolean.False);

                result2 = await lookup.call(interpreter, new BrsString("KeY2"));
        ...
        describe("lookupCI", () => {
            it("changes lookups to case sensitive mode", async () => {
                ...
                let addreplace = aa.getMethod("addreplace");
                expect(addreplace).toBeTruthy();
                await addreplace.call(interpreter, new BrsString("KEY1"), v2);
                ...
                await addreplace.call(interpreter, new BrsString("Key1"), v3);
                expect(lookupCI).toBeTruthy();

                let result = await lookup.call(interpreter, new BrsString("key1"));
                expect(result).toBe(v1);

                result = await lookup.call(interpreter, new BrsString("KEY1"));
                expect(result).toBe(v2);

                result = await lookupCI.call(interpreter, new BrsString("kEy1"));
                expect(result).toBe(v1);
