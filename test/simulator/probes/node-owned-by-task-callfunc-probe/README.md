...
| `up`     | `foreign-call` | A _different_ Task (`ForeignTask`) reads the agent back from `m.global` and calls `callFunc` on it — Task-to-Node, not render-to-Node. Previously failed before `callFunc` was even reached: `m.global.nrAgent` read back `Invalid` on `ForeignTask` — [issue #1219](https://github.com/lvcabral/brs-engine/issues/1219), a separate simulator-only bug (a field another task adds to `m.global` _after_ this task's own launch was invisible to it) unrelated to this probe's own fix. **Fixed** (`sgRoot.mGlobal.setRemoteProxy(true)` in `NodeFactory.ts`'s `initializeTask`): now succeeds, `result=77`, no crash. |
...
`task-call`, and `render-call` (#1218's fix) all succeed with no crash, and `foreign-call` also
succeeds there without any engine change (the issue #1219 read-back bug never existed on device).
`foreign-call` was re-verified against the simulator only (not re-run on device) once #1219 was
fixed there too: `rewind`, `up`, `down` in sequence all now succeed with no crash, matching device
