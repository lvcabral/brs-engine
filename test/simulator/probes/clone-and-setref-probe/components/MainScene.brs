' Render-thread half of the probe. Main.brs runs the same checks on the MAIN thread, so the two
' traces isolate whatever is thread-context dependent.
'
' `ifSGNodeField` says CanGetRef/GetRef "may only be called on the render thread", which would explain
' a device answering canGetRef = false from source/Main.brs. C4/C5/C6 test that directly. C1..C3 re-run
' the clone checks here too, in case S8's "clone of a custom component loses its fields" was the same
' main-thread artifact rather than a property of clone().
sub init()
end sub

function runTests(ctx as String) as String
    runGuarded(ctx)
    return "ok"
end function

' Each half is isolated: on a device a render-thread-only API called from the main thread throws, and
' that must not stop the other context's block from running.
