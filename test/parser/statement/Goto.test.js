...
describe("parser goto statements", () => {
    it("parses standalone statement properly", () => {
        let { statements, errors } = brs.Parser.parse([token(Lexeme.Goto, "goto"), identifier("SomeLabel"), EOF]);
        expect(errors.length).toEqual(0);
        expect({ errors, statements }).toMatchSnapshot();
    });

    it("detects labels", () => {
        let { statements, errors } = brs.Parser.parse([identifier("SomeLabel"), token(Lexeme.Colon, ":"), EOF]);
        expect(errors.length).toEqual(0);
    ...
    it("do not allow label inside try catch", () => {
        ...
        `);
        let { errors } = brs.Parser.parse(tokens);
