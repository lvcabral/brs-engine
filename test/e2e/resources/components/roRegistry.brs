sub main()
    rs = CreateObject("roRegistrySection", "Transient")
    opt = rs.read("option1")
    print opt = ""
    print rs.write("option1",opt + ".value")
    print rs.read("option1")
    print rs.write("option2","other")
    print rs.GetKeyList()
    print rs.exists("option1")
    print rs.exists("option2")
    print rs.exists("option3")
    print rs.readMulti(["option1", "option2", "option3"])
    ' writeMulti stores only string values; non-strings (e.g. invalid) leave the key untouched
    print rs.writeMulti({option4: "text", option5: invalid})
    print rs.exists("option4")
    print rs.exists("option5")
    rs.flush()
    reg = CreateObject("roRegistry")
    print reg.getSectionList()
	print reg.getSpaceAvailable()
end sub