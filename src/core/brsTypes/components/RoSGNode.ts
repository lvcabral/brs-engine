        ...
        interpreter.inSubEnv(async (subInterpreter) => {
            ...
            try {
                if (callable.getFirstSatisfiedSignature([event])) {
                    ...
                    await callable.call(subInterpreter, event);
                } else {
                    await callable.call(subInterpreter);
                }
            } catch (err) {
    ...
    if (typeDef) {
        ...
        while (typeDef) {
            ...
            await interpreter.inSubEnv(async (subInterpreter) => {
                ...
                await addFields(subInterpreter, node!, typeDef!);
                return BrsInvalid.Instance;
            }, currentEnv);
...
async function addFields(interpreter: Interpreter, node: RoSGNode, typeDef: ComponentDefinition) {
