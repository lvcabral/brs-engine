...
3. **Cross-thread serialization — `jsValueOf`/`fromAssociativeArray` container cycles (`factory/Serializer.ts`).** `fromSGNode` dedupes revisited **nodes** via its `visited` WeakSet, but a plain AA/array cycle (an app storing a back-reference to its own task `m`, e.g. `this._plugin = m` in a helper constructor — a common analytics-SDK pattern) recurses `jsValueOf ↔ fromAssociativeArray` forever. Worse, the throw aborts `checkTaskRun` before `started` is set, so the render loop retries the runaway serialization every frame → OOM. Guard: containers share the visited set **per descent path** (added on entry, removed in `finally`) — a cyclic reference is dropped (`null`/`{}`, one-shot warning), while a container legitimately referenced from two sibling paths still serializes both times. Don't "simplify" to whole-pass tracking like nodes: that would silently drop diamond-shaped shared data. Regression: "circular container references" in `test/extensions/scenegraph/NodeSerialization.test.js`.
