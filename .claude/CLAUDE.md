...
Two distinct SceneGraph paths can recurse until the JS stack overflows; both surface as `roSGNode.Set (unhandled exception): "...": Maximum call stack size exceeded`. They are unrelated — diagnose which before "fixing" the other. A real overflow's BrightScript backtrace is misleading (often one frame, because `Field.executeCallbacks` pops frames as the error unwinds); capture the **native JS stack** mid-recursion (a temporary depth tripwire dumping `new Error().stack` via `BrsDevice.stderr`).
...
### Debugger halts every thread (`DataType.DBT` — must cover the blocking rendezvous waits)

When any thread enters the Micro Debugger (a `STOP`, a crash in developer mode, or an external `BREAK`), **all** threads must freeze until the session ends, matching a real Roku. The handshake is one shared-array slot, `DataType.DBT` ("Debug Thread id"): `BrsDevice.notifyDebugStarted()` writes the debugging thread's id there; other threads observe it via **`BrsDevice.pauseIfDebugging()`** (blocks in `Atomics.wait(sharedArray, DBT, dbt)` while `dbt >= 0 && dbt !== threadId`); `notifyDebugEnded()` clears it. `pauseIfDebugging()` is the **single definition** — `checkBreakCommand` (per-statement / `roMessagePort.wait` poll) and the Task rendezvous waits both call it.

The subtle part: `DBT` is only *polled*, and a thread already parked in the multi-threaded rendezvous is blocked in **`Atomics.wait` on its own response buffer** (`SharedObject.waitVersion`), which can't simultaneously watch `DBT`. So every blocking rendezvous loop in `nodes/Task.ts` (`requestFieldValue`, `requestMethodCall`, `waitForFieldAck`, and the idle `getNewEvents`) **caps each sleep to `RENDEZVOUS_POLL_MS` (100 ms)** and drives the real timeout off the deadline, re-checking `pauseIfDebugging()` at the top of each iteration. Without the cap, a Task mid-rendezvous when the render thread breaks waits out the full ~10 s `rendezvousTimeout` and throws a **spurious "Rendezvous timeout"** that crashes the app out from under the debugger. **Invariants:** (1) `pauseIfDebugging()` is checked *before* the `remaining <= 0` timeout break, and a pause **resets the deadline** so debug time never counts toward the rendezvous timeout; (2) the pause path **must not re-send** the request (it was sent once before the loop) — just reset and re-wait; (3) the debugger's `EXIT` command calls `notifyDebugEnded()` (not only `CONT`), so paused threads are released on teardown and `DBT` is never left set (deadlock). Normal rendezvous latency is unaffected — a stored response still wakes the waiter immediately via `notify`; the cap only matters when nothing arrives. Known gap: `debugThreads` still lists only the current thread (no thread-switch command), so the other halted threads can't yet be inspected from the prompt.
