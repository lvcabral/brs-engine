...
**Build-order dependency (overwrite chain):** each package build zips a core-only `common.zip` from `src/core/common/`, but the **scenegraph** build creates a superset (core + `src/extensions/scenegraph/common/**` — Metropolis fonts, `system-fonts.json`, focus/dialog/keyboard 9-patches, video overlays) and its `afterEmit` hook copies that merged zip **over** `packages/{browser,node}/assets/common.zip`. So `npm run build:cli` alone **downgrades** the node package's `common.zip` to core-only — always run `npm run build:sg` after `build:cli` (the full `npm run build` orders this correctly; `prepublishOnly` too). Symptom of a stale/core-only zip: ~20 `test/cli/cli.test.js` failures with `ENOENT: /common:/fonts/system-fonts.json`.
