...
A `Callable` created without a name (an anonymous `function`/`sub`) is auto-named `$anon_N` (`Callable.ts`), and `toString()`/`toStr()` reports it as `<Function: $anon_N>`. That name must stay **resolvable back to the same callable**, because some BrightScript libraries identify a function argument by extracting its name from `toStr()` and passing that name to `observeField`/`observeFieldScoped` (notably [rokucommunity/promises](https://github.com/rokucommunity/promises), which backs every Rooibos `@SGNode`/node test suite — the observer registered this way is what advances its promise chain). To support this, every anonymous `Callable` is entered into a **capped registry** (`anonRegistry`, `resolveAnonymousCallable`), and `Interpreter.getCallableFunction` falls back to it for `$anon_*` names when normal variable resolution fails. **Invariants:** keep the anonymous name **deterministic** (a plain counter — do *not* switch to a random/location id: random breaks stable `toStr()` output and the `roFunction` e2e snapshot; memory is bounded by the registry cap, not the id scheme), and keep the registry **capped** (anonymous callables are minted per evaluation, so an unbounded map would leak). Regression coverage: the "Resolves an anonymous function observer registered by its toStr() name" case in `test/cli/cli.test.js` (`anon-observer-app`).
...
- **Never embed an assignment inside a larger expression** — put it on its own statement. This includes logical-assignments: write `x ??= []; x.push(y);` and `x ??= new Foo(); return x;`, **not** `(x ??= []).push(y)` or `return (x ??= new Foo())`. This is flagged by SonarCloud/SonarQube (`typescript:S1121`, "Assignments should not be made from within sub-expressions"), which runs on every PR and is **not** caught by ESLint — so it won't surface locally. The same applies to `=` inside a `return`, condition, argument, or ternary.
